...
p=4*S1;      % number of fixed nodes P
...
for i=1:S1
    ...
    P(1:3,2*S1+i)=[Rg*sin(phi +alpha/2); Rg*cos(phi +alpha/2); 0]; % outer ground points
    P(1:3,3*S1+i)=[Rg*sin(phi -alpha/2); Rg*cos(phi -alpha/2); 0];
end
...
for i=1:S1, j=mod(i,S1)+1; k=mod(i+1,S1)+1; 
  ...
  C(6*S1+i,   i)=1; C(6*S1+i,4*S1+i)=-1;    % strings from payload to ground (set 1)
  C(7*S1+i,   i)=1; C(7*S1+i,5*S1+i)=-1;    % strings from payload to ground (set 2)
end
